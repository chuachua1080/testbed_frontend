...
var Device = IconElement.extend({
	init: function(editor, name, iconsrc, iconsize, pos) {
		...
		this.paint();
		this.isDevice = true;
		this.editor.ajaxDeviceCreate({"type": this.baseName(), "pos":(pos.x-editor.paletteWidth)+","+pos.y, "name": name}, function(res) {});
	},
	getElementType: function () {
		return "device";
	},
...
var Editor = Class.extend({
	...
	ajaxDeviceCreate: function(dict,func){
		var data = dict;
		var editor = this;
		this._ajax("top/"+topid+"device_create",data,func);
	}
	...
	paintPalette: function() {
		...
		this.icon = this.g.image(basepath+"images/logo.jpg", 1, 5, this.paletteWidth-6, 79/153*(this.paletteWidth-6));
		this.icon.parent = this;
		...
		y+=5;
		this.openVZPrototype = new OpenVZDevice(this, "Ö»ú", {x: this.paletteWidth/2, y: y+=50});
